				...
				for (Entry<Long, HashMap<Long,Item>> e : this.hitsByScale.entrySet()) {
					if (!e.getValue().isEmpty() && e.getValue().size() >= this.select) {
						...
						if (autoresolve) {
							...
							throw new RuleException("Rule as either empty or less results for period "+e.getKey()+" than number of files to select. See -"+org.alb.tools.file.Constants.AUTO_OPT+" option");
						}
					}
				}
			}
